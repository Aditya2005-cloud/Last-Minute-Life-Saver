...
export default function Insights({
  ...
  const totalEstimatedFocusMinutes = completedTasks.reduce((sum, t) => sum + t.estimatedMinutes, 0);
  const activeEstimatedFocusMinutes = activeTasks.reduce((sum, t) => sum + t.estimatedMinutes, 0);

  const highestStreak = habits.length > 0 ? Math.max(...habits.map(h => h.streak), 0) : 0;
  const averagePanicScore = activeTasks.length > 0 
    ? Math.round(activeTasks.reduce((sum, t) => sum + (t.panicScore || 0), 0) / activeTasks.length) 
    : 0;
  ...
  const downloadProductivityCSV = () => {
    ...
    csvRows.push(`Active Focus Minutes Remaining,${activeEstimatedFocusMinutes} mins`);
    ...
    csvRows.push("ID,Title,Category,Description,Estimated Minutes,Panic Score,Due Date,Status");
    ...
    link.setAttribute("download", `productivity_export_${new Date().toISOString().split("T")[0]}.csv`);
  ...
  const badges: Badge[] = [
    {
      ...
      description: "Break down and complete 3 looming initiatives with survival plans.",
    ...
    },
    {
      id: "panic_stopper",
      name: "Panic Stopper",
      description: "Trigger the Panic Button Crisis Intercept protocol and clear your overdrive.",
    ...
    },
    {
      ...
      description: "Build consistency by reaching a 3-day streak on any priority habit.",
    ...
    },
    {
      id: "cognitive_zen",
      name: "Cognitive Zen",
      description: "Secure your calendar blocks and keep average panic thresholds below 50.",
  ...
  const [activeMetricTab, setActiveMetricTab] = useState<"rate" | "time" | "both">("both");
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const realCompleted = tasks.filter(t => t.completed && t.completedAt === dateStr);
      ...
      const realTotalCount = Array.from(new Set([...realCompleted.map(t => t.id), ...realDue.map(t => t.id)])).length;
      ...
      let finalEstMinutes = realCompleted.reduce((sum, t) => sum + (t.estimatedMinutes || 25), 0);
      let finalActMinutes = realCompleted.reduce((sum, t) => sum + (t.actualMinutes ?? t.estimatedMinutes ?? 25), 0);
  ...
  const chartSummary = useMemo(() => {
    const totalEst = chartData.reduce((sum, item) => sum + item.estimatedMinutes, 0);
    const totalAct = chartData.reduce((sum, item) => sum + item.actualMinutes, 0);
    const avgRate = Math.round(chartData.reduce((sum, item) => sum + item.completionRate, 0) / chartData.length);
    const speedDifference = totalEst - totalAct;
    const speedRatio = totalEst > 0 ? Math.round((speedDifference / totalEst) * 100) : 0;
    ...
    <div className="space-y-8" id="insights-container">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">Review completion rates, active velocity records, and unlock gamified badges.</p>
        </div>
      ...
      {/* METRIC CARD ROW */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" id="insights-metrics">
        {/* Completion Rates */}
        <div className="bg-zinc-950 border border-zinc-900 p-5 rounded-2xl relative overflow-hidden">
          ...
          <h4 className="text-[10px] font-bold text-zinc-500 font-mono uppercase tracking-widest block">Completion Ratio</h4>
          <h3 className="text-3xl font-extrabold text-white mt-2 font-display">{completionRate}%</h3>
          <p className="text-xs text-zinc-400 mt-1">{completedCount} of {totalTasks} cleared</p>
        </div>

        {/* Focus Minutes Secured */}
        <div className="bg-zinc-950 border border-zinc-900 p-5 rounded-2xl relative overflow-hidden">
          <span className="absolute top-0 left-0 right-0 h-[2px] bg-purple-500/30" />
          <h4 className="text-[10px] font-bold text-zinc-500 font-mono uppercase tracking-widest block">Focus Minutes Cleared</h4>
          <h3 className="text-3xl font-extrabold text-white mt-2 font-display">{totalEstimatedFocusMinutes}m</h3>
          <p className="text-xs text-zinc-400 mt-1">{activeEstimatedFocusMinutes}m remaining on grid</p>
        </div>

        {/* Highest Streak */}
        <div className="bg-zinc-950 border border-zinc-900 p-5 rounded-2xl relative overflow-hidden">
          <span className="absolute top-0 left-0 right-0 h-[2px] bg-orange-500/30" />
          <h4 className="text-[10px] font-bold text-zinc-500 font-mono uppercase tracking-widest block">Longest Habit Run</h4>
          <h3 className="text-3xl font-extrabold text-white mt-2 font-display">{highestStreak} Days</h3>
          <p className="text-xs text-zinc-400 mt-1">Consistent daily focus drills</p>
        </div>

        {/* Avg Panic Score */}
        <div className="bg-zinc-950 border border-zinc-900 p-5 rounded-2xl relative overflow-hidden">
          <span className="absolute top-0 left-0 right-0 h-[2px] bg-red-500/30" />
          <h4 className="text-[10px] font-bold text-zinc-500 font-mono uppercase tracking-widest block">Average Grid Panic</h4>
          <h3 className="text-3xl font-extrabold text-white mt-2 font-display">{averagePanicScore}%</h3>
          <p className="text-xs text-zinc-400 mt-1">
            {averagePanicScore >= 70 ? "Overdrive threshold reached" : "Prefrontal cortex relaxed"}
          </p>
        </div>
      </div>

      {/* PERFORMANCE TRENDS - DUAL-AXIS RECHARTS LINE GRAPH */}
      <div className="bg-zinc-950 border border-zinc-900 rounded-3xl p-6 md:p-8 space-y-6 shadow-2xl relative overflow-hidden" id="performance-trends-card">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-zinc-500 text-xs mt-1">
              Analyze daily task completion rates alongside time estimations and active speed accuracy.
            </p>
          </div>
        ...
        <div className="bg-zinc-900/40 border border-zinc-900 p-4 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="font-semibold text-white">Dynamic Completion Analytics</p>
              <p className="text-zinc-500 text-[11px] mt-0.5">
                {chartSummary.speedRatio > 0 ? (
                  <>
                    Outstanding velocity! You are averaging <span className="text-cyan-400 font-bold font-mono">{chartSummary.speedRatio}% faster</span> completion speed than estimates.
                  </>
                ) : chartSummary.speedRatio < 0 ? (
                  <>
                    Deep-dive alert. You are spending <span className="text-red-400 font-bold font-mono">{Math.abs(chartSummary.speedRatio)}% longer</span> on tasks than initial estimates.
                  </>
                ) : (
                  <>You are perfectly aligned on your focus estimations. High prefrontal calibration!</>
                )}
              </p>
            </div>
          </div>
          <div className="flex gap-4 font-mono text-[10px] text-zinc-500 bg-zinc-950/40 p-2.5 rounded-xl border border-zinc-900 shrink-0">
            <div>
              <span className="block text-[9px] uppercase font-bold text-zinc-600 mb-0.5">Estimated</span>
              <span className="text-amber-400 font-extrabold">{chartSummary.totalEst}m</span>
            </div>
            <div className="border-l border-zinc-800 pl-4">
              <span className="block text-[9px] uppercase font-bold text-zinc-600 mb-0.5">Actual Spent</span>
              <span className="text-cyan-400 font-extrabold">{chartSummary.totalAct}m</span>
            </div>
            <div className="border-l border-zinc-800 pl-4">
              <span className="block text-[9px] uppercase font-bold text-zinc-600 mb-0.5">Avg Rate</span>
              <span className="text-purple-400 font-extrabold">{chartSummary.avgRate}%</span>
            </div>
          </div>
        </div>
                  ...
                  if (active && payload && payload.length) {
                    return (
                      <div className="bg-zinc-950 border border-zinc-850 p-3 rounded-xl shadow-2xl font-sans text-xs space-y-1.5">
                        <p className="font-mono font-bold text-zinc-500 text-[10px] uppercase mb-1">{label}</p>
      ...
      <div className="space-y-4" id="badges-shelf-row">
        <div>
          ...
          <p className="text-zinc-500 text-xs">Unlock milestones as you successfully dismantle deadlines and habit constraints.</p>
        </div>
            ...
            >
              ...
              <div className="space-y-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h4 className={`text-sm font-bold truncate ${badge.isActive ? "text-white" : "text-zinc-500"}`}>
                    {badge.name}
                  </h4>
                  {badge.isActive && (
