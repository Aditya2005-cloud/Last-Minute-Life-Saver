...
export default function AgentPlan({
  ...
  // Email Automation States removed
  ...
  const handleToggleSubtask = (stepIndex: number, subtaskText: string) => {
    ...
    if (added) {
      ...
      const totalCompleted = steps.flatMap(
  ...
  const totalSubtasks = hasBreakdown
    ? currentTask!.breakdown!.tacticalSteps.flatMap((s) => s.checklist).length
    : 0;
  const completedSubtasks = hasBreakdown
    ? currentTask!.breakdown!.tacticalSteps.flatMap(
        (s) => s.completedChecklist || [],
      ).length
    : 0;
  ...
  return (
    <div className="space-y-8" id="agent-plan-container">
      ...
      {/* TASK SELECTOR PANEL */}
      <div
        className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 bg-zinc-950 border border-zinc-900 p-4 rounded-2xl"
        id="plan-selector"
      >
        <div className="flex-1">
          <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono mb-1">
            Analyze Target Initiative
          </label>
          <select
      ...
      {currentTask ? (
        ...
        >
          ...
          <div className="lg:col-span-5 space-y-6" id="plan-reasoning-col">
            <div
              className="bg-zinc-900/60 border border-zinc-850 rounded-2xl p-6 relative overflow-hidden shadow-xl min-h-[250px]"
              id="reasoning-feed-card"
            >
              ...
              {hasBreakdown ? (
                <div className="space-y-4 font-mono text-xs text-zinc-300">
                  ...
                  {currentReasoningIndex <
                    (currentTask.breakdown?.reasoningSteps?.length || 0) && (
                    <div className="flex items-center gap-1.5 px-3 py-1 text-purple-500 font-semibold tracking-pulse">
                      ...
                      <span className="text-[10px]">
                        Processing cognitive node trace...
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center text-zinc-500 font-mono text-xs">
                  ...
                  <p className="text-[10px] text-zinc-600 mt-1">
                    Click "Generate Game Plan" to trigger agent reasoning
                    models.
                  </p>
                </div>
              )}
            ...
            {hasBreakdown && currentTask.breakdown?.immediateFirstStep && (
              <div
                className="bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent border border-amber-500/25 rounded-2xl p-5 shadow-lg relative overflow-hidden"
                id="immediate-spark-card"
              >
          ...
          <div className="lg:col-span-7 space-y-6" id="plan-steps-col">
            <div
              className="bg-zinc-900/40 border border-zinc-850 rounded-2xl p-6 shadow-xl"
              id="breakdown-steps-card"
            >
              <div className="border-b border-zinc-850 pb-4 mb-6">
                ...
                <div className="space-y-2 mt-3">
                  <div className="flex items-center justify-between text-xs font-mono text-zinc-400 font-semibold">
                    ...
                    <span>
                      {completedSubtasks}/{totalSubtasks} Items (
              ...
              {hasBreakdown ? (
                <div className="space-y-8" id="timeline-flow">
                  {currentTask.breakdown?.tacticalSteps.map((step, stepIdx) => (
                    <div
                      key={stepIdx}
                      className="relative pl-6 border-l border-zinc-800 space-y-3"
                      id={`step-phase-${stepIdx}`}
                    >
                  ...
                  {currentTask.breakdown?.requiredResources && (
                    <div className="pt-6 border-t border-zinc-850 space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {currentTask.breakdown.requiredResources.map(
                          (res, rIdx) => (
                            <span
                              key={rIdx}
        ...
        <div className="bg-zinc-950 border border-zinc-900 py-16 rounded-2xl text-center">
          ...
          <p className="text-zinc-600 text-xs mt-1">
            Please create a new task on the Add Task screen first.
          </p>
        </div>
      )}
    </div>
